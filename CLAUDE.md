...
- **Notch panel background (open state, macOS 26+ only)**: 
  `notchPanelBackground(...)` modifier in `LiquidGlass.swift` layers 
  `.glassEffect(.regular, in: NotchShape)` underneath a top-down 
  `LinearGradient` from `.black` (top, opaque to ~45% of height) to 
  `.black.opacity(0)` (bottom). The black gradient camouflages the 
  hardware notch at the very top edge while the bottom of the open 
  panel reveals the wallpaper through Liquid Glass. Closed panel + 
  macOS 14/15/25 fallback both stay solid black — no behavioral 
  regression. Tune the gradient stops in `NotchPanelBackgroundModifier` 
  if the wallpaper bleed feels too strong (raise the 0.45 stop closer 
  to 0.6, or end at `.black.opacity(0.3)` instead of fully clear).
