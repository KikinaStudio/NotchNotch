...
- **Routines are the exception**: a cron job is an *object* (state, 
  ...
  schedule/counter/toggle row. Alert routines (`deliver == "local"`) get 
  a `bell.badge.fill` glyph top-right tinted `AppColors.accent` — 
  distinction is via glyph, never tint (orange is already owned by the 
  paused state). Footer shows counter OR next run (binary: 
  `completed > 0 ? counter : next`), not both. The grid's last cell is 
