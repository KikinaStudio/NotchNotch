...
- `BrainView.swift` — Three-tab panel (Brain · Tools · Tasks) for browsing Hermes knowledge and capabilities (renamed 2026-05-02 from Memory · Skills · Wiki). Tab bar uses icon+text buttons: `books.vertical` Brain · `hammer` Tools · `call.bell` Tasks (the call.bell uses the bundled Phosphor SVG pair, same loader as the burger). Each tab opens with a short French `tabIntro(_:)` one-liner. **Brain tab**: merges Memory and Wiki in a single FadingScrollView. Memory section first (`À PROPOS DE TOI` section header + flat `memoryCardCompact` list, edit/delete via hover icons routing through `chatVM.updateMemory`/`deleteMemory`). If `hasWiki`, a hairline divider + Wiki section (`À PROPOS DU MONDE` header + topic-count caption + sync button + Ask buttons routing through `onSendToChat`). Stale `selectedMemoryBlock` is guarded after deletes via `.onChange(of: allMemoryBlocks.map(\.id))`. **Tools tab**: Apps carousel in first position (Netflix-style horizontal scroll of `CuratedSkillCatalog.all` cards — brand icon + name + 2-line description + state badge), tap opens `CuratedSkillDetailView` drill-down. Below: technical skill categories (filtered from curated entries via `mappedSkillIDs`) rendered as the same Netflix-style carousels with an SF Symbol per category prefix (mapped by `iconForSkillCategory(_:)` — `media → music.note`, `productivity → briefcase.fill`, etc.). **Tasks tab**: thin wrapper that calls `tasksContent()` (a `() -> AnyView` closure passed by NotchView, currently wrapping `RoutinesView`). Refresh button reloads all sections. Calls `brainVM.refreshLightMetrics()` on appear and every 60 seconds via `Timer.publish(every: 60, on: .main, in: .common).autoconnect()` through `.onReceive`. Takes `chatVM`, `notchVM`, `onSendToChat` (auto-send, used by wiki Ask), `onPrefillChat` (no send, used by curated prompt examples), and `tasksContent`.
- `NotchView.swift` — Root view with flanking overlay buttons (42pt inset matching the input bar). **Two symmetric burgers** (refactored 2026-05-02): left burger (chat-icon `bubble.left` at the leftmost edge, deploys icons rightward toward center) and right burger (hamburger `line.3.horizontal` at the rightmost edge, deploys icons leftward toward center). Both fan toward the center of the top bar — the hardware notch sits between them. **Right burger** (settings + Brain tabs as direct entries, 4 icons left-to-right when deployed): `gearshape` (Settings) · `books.vertical` (Brain → opens panel + sets `activeTab = .brain`) · `hammer` (Tools → `activeTab = .tools`) · `call.bell` (Tasks → `activeTab = .tasks`, custom outline+filled SVG pair from Phosphor Icons MIT). The Brain/Tools/Tasks icons each open the Brain panel directly to their tab; re-clicking the same icon while it's the active tab toggles the panel closed. **Left burger** (conversation actions, 3 icons left-to-right when deployed): `plus.bubble` (new conversation, fire-and-forget action) · `bubble.left.and.bubble.right` (history panel) · `magnifyingglass` (search panel). Both burgers expand on hover with a 0.6s grace period before retract (`scheduleMenuCollapse` / `scheduleLeftMenuCollapse`). **Right burger owns the X**: regardless of which burger opened the panel (left history/search OR right settings/Brain), the close X always appears on the right burger morphing the hamburger glyph. The left burger NEVER morphs to an X — when a left-side panel (history/search) is open, the leftSlot is replaced by the panel title, so a left-side X would be hidden under the title anyway (this used to be a real bug, fixed 2026-05-02). The right burger's `showX` flag is `isAnyPanelOpen || isMenuExpanded`; click handler routes through `closeAllPanels()` which clears all panel state regardless of side. The right cluster's deployed fan stays visible when `isRightPanelOpen` (so user can switch Brain tabs without re-hovering) but is NOT auto-deployed when only a left panel is open (user must hover to switch sections). `menuButton()` renders each glyph with `.symbolVariant(active ? .fill : .none)` for SF Symbols with a `.fill` variant; the `call.bell` pair is loaded via `NSImage(contentsOf:)` + `isTemplate = true` (bypasses `Assets.car` which needs full Xcode). The Tasks tab in `brainTabsBar` uses the same SVG pair. **Top bar opacity rules**: idle burger glyph + resize at 20%, deploy lifts to `.secondary` (~55%). Resize button has its own hover state. `RecordingToastView` appears below the closed notch during voice recording with Talk/Brain Dump action buttons. Also hosts the first-run brain onboarding sheet. Standalone Routines panel REMOVED — Routines are now embedded inside Brain panel's Tasks tab via the `routinesEmbedded` private view. `NotchViewModel.isRoutinesOpen` and `openRoutines()` were deleted.
...
The Brain panel is a read-only viewer for Hermes knowledge and capabilities, opened via Brain/Tools/Tasks icons in the right burger menu (`NotchViewModel.isBrainOpen`, `openBrain()` + `brainVM.activeTab` set per icon). Three tabs: **Brain · Tools · Tasks** (renamed 2026-05-02 from Memory · Skills · Wiki to give a clearer mental model — "what your agent knows / can do / does autonomously" — and to fit 3 wider tabs around the hardware notch instead of 4 narrow ones). Each tab carries an icon next to its label: `books.vertical` Brain · `hammer` Tools · `call.bell` Tasks.
...
- **Icon sizing**: top flanking overlay icons (`NotchView` — burger 
  ...
  for consistency. The 5 burger **action icons** (history, search, 
  routines, brain, settings) are 13pt — they're rendered through 
  `menuButton()` and are slightly smaller than the surrounding overlay 
  glyphs by design (decided 2026-04-26 after stepping the constant 
  through 14 → 12 → 13 visually). The cluster `HStack(spacing: 8)` 
...
**Top bar panel title (2026-04-25, updated 2026-05-02)**: when a panel is open, 
the left slot of the top bar displays the panel title (`"Conversations"`, 
`"Settings"`, `"Search"`) instead of the chat-icon left burger. For the 
Brain panel the slot is taken by the 3 tabs (`Brain · Tools · Tasks` 
with their icons) instead — `BrainViewModel.activeTab` is `@Published` 
so NotchView can render the tab bar in `brainTabsBar` while BrainView 
reads/writes the same state. Per-panel titles are removed from the 
panel content (no internal `Text("Settings").font(.headline)` header 
anymore). The standalone Routines panel title was removed in 2026-05-02 
when Routines moved into the Tasks tab.
