...
3. Server streams SSE events (`text/event-stream`). The `onEvent` callback runs off-main; each case hops back via `Task { @MainActor in ... }` and mutates `messages[lastIndex]`: `textDelta` appends to `content`, `thinkingDelta` to `thinkingContent`, `toolCallStarted` / `toolCallCompleted` route to either `subagentActivity` (for `delegate_task` / `🤖`) or `toolCallContent`, `completed` updates `lastInputTokens`
