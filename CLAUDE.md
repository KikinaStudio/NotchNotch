...
**Tools tab — Apps carousel + technical skills carousels** (refactored 2026-05-02):

The Tools tab is a single vertical scroll of horizontal carousels (Netflix-style throughout), with the curated **Apps** carousel as the first row and the technical Hermes skills grouped by category below.

- **Apps carousel** (first row): `CuratedSkillCatalog.all` (in `BoaNotch/Models/CuratedSkillCatalog.swift`) — 6 connector entries (Spotify, Google Calendar, Gmail, Google Drive, Notion, Google Meet). Each card shows the brand icon (Simple Icons SVG tinted to the official brand color) at top-left + state indicator at top-right (green dot when connected, chevron when available), then display name (1 line) + French description (2 lines max). Tap opens `CuratedSkillDetailView` drill-down with prompt examples and a structured connect/disconnect button. State is resolved on every `brainVM.refreshLightMetrics()` via `BrainViewModel.resolveCuratedStates()` against on-disk Hermes state — no chat round-trip. The 2 always-on entries (`web-browse`, `image-generation`) were removed on 2026-05-02 because they are not connectors (no account to wire up); they remain accessible to Hermes through its built-in tools but are not surfaced as cards.
- **Technical skill carousels** (below Apps): `brainVM.technicalSkills` filters `~/.hermes/skills/<category>/<skill-name>/SKILL.md` to drop any Hermes skill claimed by a curated entry's `mappedSkillIDs`. Each surviving Hermes category renders as a horizontal carousel with an SF Symbol prefix from `iconForSkillCategory(_:)` (e.g. `media → music.note`, `productivity → briefcase.fill`, `creative → paintbrush.fill`, `software-development → hammer.fill`, `mlops → cpu`, `social-media → person.2.fill`, etc. — 25+ mappings, falls back to `tag.fill` for unknown categories). Section titles use `labelForSkillCategory(_:)` (FR translation table — `software-development → "Développement logiciel"`, `mlops → "Machine learning"`, `red-teaming → "Sécurité offensive"`, etc.) — never raw English keys with `.capitalized`. Each `skillCardCompact` carries the same category icon at top-left (22pt mono `.secondary`) so the card doesn't feel half-empty next to brand-icon Apps cards. Tap shows the raw `SKILL.md` content.

**Card sizing & edge-to-edge bleed (2026-05-02)**: Apps and skill cards are `.frame(width: 220, height: 130)` with `maxHeight: .infinity, alignment: .topLeading` on the inner content (so the `.quaternary.opacity(0.6)` background fills the 130pt slot — without this, smaller content centered in the 130pt frame leaves uneven whitespace above the visible card and breaks title→card alignment between sections). Each carousel's `ScrollView` uses `.padding(.horizontal, -42)` to negate the parent panel's outer 42pt padding, with `.padding(.horizontal, 42)` on the inner `LazyHStack` so the first card stays aligned with the section title — net effect: the scroll viewport extends edge-to-edge of the panel, cards bleed past the title's column when scrolling (Netflix-style). `FadingScrollView`'s `.mask(...)` carries `.padding(.horizontal, -200)` so the alpha mask doesn't clip these horizontally-bleeding children, and `.scrollClipDisabled()` is applied to the outer vertical ScrollView for the same reason. Don't undo any of these without breaking the bleed.
...
the index article. The Tools tab uses Netflix-style horizontal carousels 
throughout (Apps + technical skill categories).
