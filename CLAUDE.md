...
**Schedule picker (`scheduleSection`)** — replaces a free-text "When?" input so users never have to know cron syntax. Row 1 is a 5-pill frequency selector (Daily · Weekdays · Weekly · Monthly · Hourly) styled exactly like the Deliver picker (`#A8DCFC` capsule with dark text when selected). Row 2 is contextual: Daily/Weekdays show a single `DatePicker(.compact, .hourAndMinute)` prefixed by `"at"`; Weekly stacks 7 day-of-week circle pills (28pt, indices 0=Sun…6=Sat — the cron convention) above the time picker, with at least one weekday always selected; Monthly shows a `Stepper(1...28)` for day-of-month next to the time picker; Hourly shows interval pills (1h · 2h · 3h · 4h · 6h · 12h). Row 3 is a `humanSchedule(composedCron)` preview caption so users see the rendered result. `composedCron` is a computed property mapping the selections to a 5-field cron string (`"\(m) \(h) * * *"` daily, `"\(m) \(h) * * 1-5"` weekdays, `"0 */N * * *"` hourly, etc.) — that cron expression is sent to Hermes verbatim. `customFormValid` only checks the prompt is non-empty since the schedule always has a default. Layout fits within the standard panel (~556pt content width) without crowding — DO NOT enlarge the panel to accommodate this UI; if a future addition is too tight, redesign instead (user explicitly rejected enlarging the notch panel for the schedule UI on 2026-04-24).
